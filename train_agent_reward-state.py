...
if len(cmd_input) > 1:
    ...
    AGENT_SETTINGS = "SARSA_TFFT_2-2-0-0-2-32_7000_3000"
...
if __name__ == "__main__":
    ...
    for episode in tqdm(range(Episodes), desc="Episodes"):
        ...
        if DIST or LOCATION:
            State_tmp.append(list([dist_discrete[path_idx][data_idx]]))
        else:
            State_tmp.append(["N/A"])
        ...
        if LOCATION:
            State_tmp.append(list([angle_discrete[path_idx][data_idx]]))
        else:
            State_tmp.append(["N/A"])

        State_tmp.append(['low'])
        R=0
        ...
        for n in range(chunksize):
            ...
            if ADJ:
                State.state = State.build_state_reward(previous_beam_nr, current_state_parameters, previous_action, R)
            ...
            if METHOD == "SIMPLE":
                ...
                Agent.update_TD(helpers.state_to_index_reward(previous_state),
                                previous_action,
                                R,
                                helpers.state_to_index_reward(State.state),
                                action_index,
                                end=end)
