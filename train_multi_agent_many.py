        ...
        try:
            if "NLOS" in channel_settings["scenarios"][0]:
                ...
                helpers.dump_hdf5(data_reward, 'Results/',
                                  f'{CASE}_NLOS_{RESULT_NAME}_{eps}_{alpha}_{gamma}_{weight}_{agent_nr}_results.hdf5')
            else:
                # helpers.dump_pickle(data_agent, 'Results/', f'{CASE}_LOS_{RESULT_NAME}_results.pickle')
                helpers.dump_hdf5(data_reward, 'Results/',
                                  f'{CASE}_LOS_{RESULT_NAME}_{eps}_{alpha}_{gamma}_{weight}_{agent_nr}_results.hdf5')
        except OSError as e:
            print(e)
            print("Saving to root folder instead")
            if "NLOS" in channel_settings["scenarios"][0]:
                # helpers.dump_pickle(data_agent, '', f'{CASE}_NLOS_{RESULT_NAME}_results.pickle')
                helpers.dump_hdf5(data_reward, '', f'{CASE}_NLOS_{RESULT_NAME}_{eps}_{alpha}_{gamma}_{weight}_{agent_nr}_results.hdf5')
            else:
                # helpers.dump_pickle(data_agent, '', f'{CASE}_LOS_{RESULT_NAME}_results.pickle')
                helpers.dump_hdf5(data_reward, '', f'{CASE}_LOS_{RESULT_NAME}_{eps}_{alpha}_{gamma}_{weight}_{agent_nr}_results.hdf5')
